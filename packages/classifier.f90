subroutine genese_classifier(file,nchar, nlin,sea)
    ...
    if(cyc(1,2)<=-20.)then
      ...
      ! check if cyclone is a pure extratropical
        ...
        if(nex==6)then
            if(count_phases>=4 .and. extrp_flex<=3 .and. o_ex<=2 .and. cyclone_gen=='ANALYSIS')then
                cyclone_gen='EXTRATROPICAL'
            endif
...
subroutine extratropical(lat,b,vtl,vtu,class_ax,nlin,time, extrp_flex, o_ex)
    ...
    if(lat<=-20.)then
        ...
        if((abs(b)>=25. .or. time==1) .and. vtl<-50. .and. vtu<-80.)then
            class_ax(time)=1
                
        elseif(abs(b)>=20. .and. vtl<-30 .and. vtu<-60 .and. time<=6)then
            class_ax(time)=1
